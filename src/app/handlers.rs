                ...
                if let Some(idx) = app.selected_local_file_index {
                    if idx > 0 {
                        ...
                        app.selected_download_index =
                            Some(app.download_manager.task_order.len() - 1);
                    }
            ...
            match app.state {
                AppState::Settings => match code {
                    ...
                    KeyCode::Down | KeyCode::Char('j') => {
                        let current = app.settings_state.selected().unwrap_or(0);
                        let next = (current + 1)
                            % crate::tui::components::settings::SettingItem::all().len();
                        app.settings_state.select(Some(next));
                    }
                ...
                AppState::Downloads => match code {
                    ...
                    KeyCode::Enter => {
                        if app.selected_local_file_index.is_some()
                            || app.selected_download_index.is_some()
                        {
                            app.previous_app_state = app.state;
                            app.state = AppState::ActionMenu;
                        }
                        ...
                        match action.action {
                            ...
                            AppAction::DeleteSelectedLocalFiles => {
                                let indices: Vec<usize> =
                                    app.selected_local_file_indices.iter().cloned().collect();
                                for &idx in &indices {
                                    if let Some(file) = app.local_files.get(idx) {
                                ...
                                actions::refresh_local_files(app);
                                app.status_message =
                                    Some(format!("Deleted {} files.", indices.len()));
                                app.state = app.previous_app_state;
                            }
                            ...
                            AppAction::CleanupLocalGarbage => {
                                let download_path_buf =
                                    local::resolve_path(&app.download_directory);
                                let download_path = download_path_buf.as_path();
                                match local::cleanup_garbage(download_path) {
                                    Ok(count) => {
                                        app.status_message =
                                            Some(format!("Cleaned {} garbage files.", count));
                            ...
                            _ => {
                                ...
                                let target_video = if effective_state == AppState::Downloads {
                                    if let Some(idx) = app.selected_download_index {
                                        if let Some(task_id) =
                                            app.download_manager.task_order.get(idx)
                                        {
                                            app.download_manager
                                                .tasks
                                                .get(task_id)
                                ...
                                if let Some(video) = target_video {
                                    ...
                                    match action.action {
                                        ...
                                        AppAction::CopyUrlOrId => {
                                            let text_to_copy =
                                                if effective_code == KeyCode::Char('i') {
                                            ...
                                            if let Some(clipboard) = &mut app.clipboard {
                                                if clipboard.set_text(text_to_copy).is_ok() {
                                                    ...
                                                    app.status_message =
                                                        Some(format!("Failed to copy {}.", label));
                                                }
                                            } else {
                                                app.status_message =
