...
pub fn convert_json_to_netscape(json_path: &Path, netscape_path: &Path) -> Result<()> {
    ...
    let cookies: Vec<JsonCookie> = serde_json::from_str(&content)
        .map_err(|e| {
            log::warn!(
                "Initial JSON parsing error: {}. Attempting to clean content...",
                e
            );
            e
        })
    ...
    for cookie in cookies {
        ...
        let domain_flag = if cookie.host_only.unwrap_or(false) {
            "FALSE"
        } else {
            "TRUE"
        };
        let secure_flag = if cookie.secure.unwrap_or(false) {
            "TRUE"
        } else {
            "FALSE"
        };
        ...
        output.push_str(&format!(
            "{}\t{}\t{}\t{}\t{}\t{}\t{}\n",
            domain, domain_flag, path, secure_flag, expires, name, value
        ));
        valid_count += 1;
    }
    ...
    if valid_count == 0 && skipped_count > 0 {
        anyhow::bail!(
            "No valid cookies found in JSON file ({} entries skipped). Please check the file format.",
            skipped_count
        );
    }

    log::info!(
        "Converted {} cookies (skipped {} invalid entries)",
        valid_count,
        skipped_count
    );
